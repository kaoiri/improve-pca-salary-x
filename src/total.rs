...
impl Total {
    ...
    pub fn diff_work_time(&self) -> anyhow::Result<Time> {
        let diff = self
            .rounded_work_time
            .peek()?
            .clone()
            .sub_allow_negative(self.total_work_time.peek()?)
            .clone();
        Ok(diff)
    }
    ...
    pub fn export_to_csv(&self) -> String {
        ...
        buf.push(self.diff_work_time().unwrap_or(Time::new(0, 0)).to_string());
